...
def main():
    ...
    img_ori = skimage.io.imread('img-prior-in/kate.png')
    img_msk = skimage.io.imread('img-prior-in/kate_mask.png') / 255
    img_cor = np.expand_dims(img_ori * np.expand_dims(img_msk, 2), 0) / 255. - 1.

    msk_in  = np.expand_dims(np.array([img_msk, img_msk, img_msk]).transpose((1, 2, 0)), 0)

    if len(img_cor.shape) < 4: img_cor = np.expand_dims(img_cor, 3)
    img_w, img_h, img_c = img_cor.shape[1], img_cor.shape[2], img_cor.shape[3]

    batch_size = 1
    tf.reset_default_graph()
    tnsr_img_cor = tf.placeholder(dtype=tf.float32, shape=[batch_size, img_w, img_h, img_c])
    tnsr_img_msk = tf.placeholder(dtype=tf.float32, shape=[batch_size, img_w, img_h, img_c])
    ...
    mse_loss= tf.reduce_mean(tf.squared_difference(tnsr_img_cor, img_gen * tnsr_img_msk))
    ...
    with tf.Session() as sess:
        ...
        for it in xrange(4000):
            time_cnt = time.time()
            _img_gen, _mse_loss, _ = sess.run([img_gen, mse_loss, gen_train_op], \
                                              feed_dict={tnsr_rnd_in: rnd_in, tnsr_img_cor: img_cor, \
                                                         tnsr_img_msk: msk_in})

            if it % 500 == 0:
                save2img(_img_gen[0], "%s/output-it%03d.png" % (it_out_path, it))
                if it==0: save2img(img_cor[0], "%s/noisy-in.png" % (it_out_path, ))
