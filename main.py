...
def unet_conv_block(inputs, nch, is_training=False):
...
def unet_upsample(inputs, is_training=False, nlayers=4):
    outputs = {}
    ...
    _tmp = tf.layers.batch_normalization(_tmp, training=is_training)
...
def main():
    ...
    noisy_img = np.expand_dims(skimage.io.imread('img-prior-in/snail.jpg')[2:258]/255., 0) # norm to [0, 1]
    ...
    with tf.variable_scope('generator') as scope:
        with tf.device('/gpu:0'):   # change to '/cpu:0' or remove this line if you do not have a compatible GPU
            img_gen = unet_upsample(tnsr_rnd_in, 4, is_training=True)
